...
- `pixel_map.json` は共通で、1 枚の Canvas 上に Display A/B を左右に並べて描画
- 単色生成（RGB スライダー）・ランダム生成・ブラシ描画に対応し、送信操作は A/B 同期で実行
- 連続送信は 2 台のディスプレイを同時に行うモードのみ用意（frame_id を毎回+1）
- 共通プレビュー Canvas 上でブラシ描画でき、HSV スライダーと半径スライダーで色・サイズを直感的に調整可能
...
3. 「フレーム生成」のスライダーで単色を作るかランダム生成を押し、またはプレビュー下のブラシ設定で Canvas を直接塗り、共通の「両方へ 1 回送信」ボタンで同じ frame_id を送出します（送信後に+1）。
4. `frame_id` 入力欄は 2 台で共有され、1回送信・自動送信とも送信が成功したタイミングで +1 されます。
5. Canvas 上でドラッグすると現在のブラシ色/半径でLEDを塗れます。必要に応じてブラシ RGB 値と半径スライダーを調整してください。
6. ログ欄で ACK/エラーを確認し、必要なら `ログをクリア` で消去。
